...
N_POSE_LANDMARKS = (
    max([index for connection in POSE_CONNECTIONS for index in connection]) + 1
)
N_HAND_LANDMARKS = (
    max([index for connection in HAND_CONNECTIONS for index in connection]) + 1
)
...
N_BOTH_LANDMARKS = (
    max([index for connection in BOTH_CONNECTIONS for index in connection]) + 1
)

ALL_CONNECTIONS = sorted(POSE_CONNECTIONS) + [
    (N_POSE_LANDMARKS + start, N_POSE_LANDMARKS + end)
    for start, end in BOTH_CONNECTIONS
]

N_ALL_LANDMARKS = (
    max([index for connection in ALL_CONNECTIONS for index in connection]) + 1
)
...
def _get_pose_connection_color(start, end):
    return (
        ...
        else _get_default_connection_color(start, end)
    )
...
def _get_both_connection_color(start, end):
    return (
        _get_hand_connection_color(start, end)
        if start < N_HAND_LANDMARKS and end < N_HAND_LANDMARKS
        else _get_hand_connection_color(
            start - N_HAND_LANDMARKS, end - N_HAND_LANDMARKS
        )
    )
...
def _get_all_connection_color(start, end):
    if start < N_POSE_LANDMARKS and end < N_POSE_LANDMARKS:
        ...
        start < N_POSE_LANDMARKS + N_HAND_LANDMARKS * 2
        and end < N_POSE_LANDMARKS + N_HAND_LANDMARKS * 2
    ):
        color = _get_both_connection_color(
            start - N_POSE_LANDMARKS, end - N_POSE_LANDMARKS
        )
...
def infer_connections(n_landmarks):
    return (
        POSE_CONNECTIONS
        if n_landmarks == N_POSE_LANDMARKS
        else HAND_CONNECTIONS
        if n_landmarks == N_HAND_LANDMARKS
        else BOTH_CONNECTIONS
        if n_landmarks == N_BOTH_LANDMARKS
        else ALL_CONNECTIONS
        if n_landmarks == N_ALL_LANDMARKS
        else None
    )
...
def plot_landmarks(
    ...
    get_connection_color = (
        _get_hand_connection_color
        if connections == HAND_CONNECTIONS
        else _get_pose_connection_color
        if connections == POSE_CONNECTIONS
        else _get_both_connection_color
        if connections == BOTH_CONNECTIONS
        else _get_all_connection_color
        if connections == ALL_CONNECTIONS
        else _get_default_connection_color
    )
    ...
    for connection in connections:
        ...
        ax.plot3D(
            ...
            label=(
                ...
                if connection in [(3, 4), (24, 25), (36, 37), (57, 58)]
                else "Pinky"
                if connection in [(19, 20), (40, 41), (52, 53), (73, 74)]
                else None
            ),
            alpha=alpha,
        )
...
def _move_hands_apart(landmarks, distance=0.2, scale=1.0):
    landmarks *= scale

    if landmarks.shape[-2] == N_BOTH_LANDMARKS:

        landmarks[..., :N_HAND_LANDMARKS, 0] += distance / 2  # left  hand
        landmarks[..., N_HAND_LANDMARKS:, 0] -= distance / 2  # right hand

    return landmarks


def _move_hands_to_wrists(landmarks, scale=1.0):

    if landmarks.shape[-2] == N_ALL_LANDMARKS:

        # Left hand
        left_wrist = landmarks[
            ..., POSE_LEFT_WRIST_INDEX : POSE_LEFT_WRIST_INDEX + 1, :
        ]
        left_hand_landmarks = landmarks[
            ..., N_POSE_LANDMARKS : N_POSE_LANDMARKS + N_HAND_LANDMARKS, :
        ]
        left_hand_landmarks *= scale
        left_hand_landmarks -= left_hand_landmarks[
            ..., HAND_WRIST_INDEX : HAND_WRIST_INDEX + 1, :
        ]
        left_hand_landmarks += left_wrist

        # Right hand
        right_wrist = landmarks[
            ..., POSE_RIGHT_WRIST_INDEX : POSE_RIGHT_WRIST_INDEX + 1, :
        ]
...
def _move_hands(landmarks, connections, hands_location, scale=1.0, distance=0.2):
...
def plot_multi_frame_landmarks(
    ...
    for i, landmarks in enumerate(multi_frame_landmarks.copy()):
        ...
        fig, ax = plot_landmarks(
            ...
            elev=elev,
            azim=azim,
            landmarks_color=_get_default_connection_color(),
            landmark_size=0,
            alpha=0.5,
        )
        fig, ax = plot_landmarks(
...
def multi_frame_landmarks_to_image(
    multi_frame_landmarks,
    connections=None,
    landmarks_color=Colors.RED,
    axis_lims=None,
...
def multi_frame_landmarks_to_video(
    ...
    return DataVideoClip(timesteps, data_to_frame, fps=fps)
